...
def upload_to_vk(video_path, description="", title=""):
    ...
    try:
        ...
        # Prepare message
        message = description if description else "🧘 Monk Tashirok Daily!\n\n#buddhism #zen #mindfulness #wisdom"
        ...
        video = upload.video(
            ...
            name=title or 'Monk Tashirok Video',
...
def main():
    ...
    description = sys.argv[2] if len(sys.argv) > 2 else "Daily Wisdom from Monk Tashirok 🧘"
    title = sys.argv[3] if len(sys.argv) > 3 else "Monk Tashirok"
